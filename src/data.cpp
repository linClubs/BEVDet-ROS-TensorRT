...
{
    // cv::Mat img = cv::imread(image_names, cv::IMREAD_UNCHANGED);
    cv::Mat img = cv::imread(image_names);
...
int read_sample_cv(std::vector<std::string> &imgs_file, std::vector<std::vector<char>> &imgs_data)
{
    ...
    {   
        ...
        if(read_image_cv(imgs_file[i], imgs_data[i]))
        {
            return EXIT_FAILURE;
        }
    }
